 ...
 *  - test_3: SubscribeCoroutineWorker → pid same + isRetry = true ⇒ Result.retry.
 *  - test_4: SubscribeCoroutineWorker → pid same + isRetry = false ⇒ Result.success.
 *  - test_5: UpdateCoroutineWorker → pid same + isRetry = true ⇒ Result.retry.
 *  - test_6: UpdateCoroutineWorker → pid same + isRetry = false ⇒ Result.success.
 ...
 *  - test_10: pidChanged branch (subscribe) ⇒ Result.success, isRetry not called.
 *  - test_11: pidChanged branch (update) ⇒ Result.success, isRetry not called.
...
class WorkersInstrumentedTest {
    ...
    private fun updateWorker(samePid: Boolean = true): Worker.TokenUpdateCoroutineWorker {
        val data = Data.Builder()
            .putInt(PID, if (samePid) Process.myPid() else -1)
            .build()
        return TestListenableWorkerBuilder<Worker.TokenUpdateCoroutineWorker>(context)
            .setInputData(data)
            .build()
    }
    ...
    /** - test_3: SubscribeCoroutineWorker → same pid + isRetry = true ⇒ Result.retry. */
    @Test
    fun subscribeWorker_retry_path_when_isRetry_true_and_pid_same() = runTest {
    ...
    /** - test_4: SubscribeCoroutineWorker → same pid + isRetry = false ⇒ Result.success. */
    @Test
    fun subscribeWorker_success_path_when_isRetry_false_and_pid_same() = runTest {
    ...
    /** - test_5: UpdateCoroutineWorker → same pid + isRetry = true ⇒ Result.retry. */
    @Test
    fun updateWorker_retry_path_when_isRetry_true_and_pid_same() = runTest {
    ...
    /** - test_6: UpdateCoroutineWorker → same pid + isRetry = false ⇒ Result.success. */
    @Test
    fun updateWorker_success_path_when_isRetry_false_and_pid_same() = runTest {
    ...
    /** - test_10: SubscribeCoroutineWorker → pidChanged ⇒ Result.success, isRetry not called. */
    @Test
    fun subscribeWorker_pidChanged_skips_isRetry_and_returns_success() = runTest {
        coEvery { PushSubscribe.isRetry(any(), any()) } returns true
    ...
    /** - test_11: UpdateCoroutineWorker → pidChanged ⇒ Result.success, isRetry not called. */
    @Test
    fun updateWorker_pidChanged_skips_isRetry_and_returns_success() = runTest {
        coEvery { TokenUpdate.isRetry(any(), any()) } returns true
