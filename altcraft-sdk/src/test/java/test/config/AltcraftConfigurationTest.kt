 ...
 *
 * Positive scenarios:
 *  - test_1: All known providers → toEntity() returns non-null; providerPriorityList preserved 1:1.
 ...
 *
 * Negative scenarios:
 *  - test_4: Single unknown provider → toEntity() returns null; Events.error called with code 470 and provider name.
 *  - test_5: Mixed known + unknown providers → toEntity() returns null; error lists all invalid.
 *  - test_6: Case mismatch against constants → toEntity() returns null; error mentions the mismatched provider.
 *  - test_7: Empty apiUrl with valid providers → toEntity() returns null; error mentions "apiUrl".
 */
class AltcraftConfigurationTest {
    ...
    private companion object {
        ...
        private const val MSG_EXPECT_ENTITY_EMPTY_LIST = "Expected non-null entity when providerPriorityList is empty"
        private const val MSG_EXPECT_LIST_MAPPED_1_TO_1 = "Expected providerPriorityList to be mapped 1:1"
        private const val MSG_EXPECT_NULL_UNKNOWN_PROVIDER = "Expected null when providerPriorityList contains unknown provider"
        private const val MSG_EXPECT_NULL_MIXED = "Expected null for mixed list with unknown providers"
        private const val MSG_EXPECT_NULL_CASE_MISMATCH = "Expected null when provider case does not match constants"
        private const val MSG_EXPECT_NULL_EMPTY_API_URL = "Expected null when apiUrl is empty even if providers are valid"
    }
    ...
    fun setUp() {
        ...
        mockkObject(Events)
        every { Events.error(any(), any<Pair<Int, String>>()) } returns mockk<DataClasses.Error>(relaxed = true)
    ...
    /** - test_1: All known providers → entity created; list mapped 1:1. */
    @Test
    ...
    /** - test_2: Subset of known providers → entity created; list mapped 1:1. */
    @Test
    ...
    /** - test_3: Empty providerPriorityList → entity created; list is empty. */
    @Test
    ...
    /** - test_4: Single unknown provider → null; Events.error contains provider name. */
    @Test
    ...
    /** - test_5: Mixed known + unknown providers → null; error lists all invalid. */
    @Test
    fun toEntity_invalid_mixed_known_and_unknown_fails_and_logs_all_invalid() {
        val cfg = AltcraftConfiguration.Builder(
            apiUrl = API_URL,
            providerPriorityList = listOf(FCM_PROVIDER, INVALID_FIREBASE, RUS_PROVIDER, INVALID_UNKNOWN)
        ).build()
            ...
            Events.error(
                ...
                match<Pair<Int, String>> { (code, msg) ->
                    code == 470 &&
                            msg.contains("invalid config", ignoreCase = true) &&
                            msg.contains(INVALID_FIREBASE) &&
                            msg.contains(INVALID_UNKNOWN)
                }
            )
        }
    }

    /** - test_6: Case mismatch against constants → null; error mentions mismatched provider. */
    @Test
    fun toEntity_invalid_case_mismatch_fails() {
        val candidate = FCM_PROVIDER.lowercase()
        val providers = if (candidate == FCM_PROVIDER) listOf(FCM_PROVIDER + "_") else listOf(candidate)
    ...
    /** - test_7: Empty apiUrl with valid providers → null; error mentions "apiUrl". */
    @Test
